...
const getDbInfo = async () => {
    return await Dog.findAll({
      include: {
          model: Temperament, //para cuando cree una raza nueva me traiga los temperamentos
...
router.get('/:id', async (req, res) => {
    ...
    if (id) {
        let dogId = await dogsTotal.filter(el => el.id == id) // "==="" no funciona, ni idea el porque
        dogId.length?
